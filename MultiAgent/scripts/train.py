...
def train():
    ...
    args.add_args("env_name", "navigation")
    args.add_args("n_agents", 4)
    
    # batch settings
    args.add_args("n_iterations", 100)
    args.add_args("frames_per_batch", 6000)
    args.add_args("max_steps", 100)
    args.add_args("minibatch_size", 100)
    args.add_args("total_frames", args.args["n_iterations"] * args.args["frames_per_batch"])
    args.add_args("n_envs", args.args["frames_per_batch"] // args.args["max_steps"])
    args.add_args("n_minibatches", args.args["frames_per_batch"] // args.args["minibatch_size"])

    # actor model settings
    args.add_args("centralised_actors", False)
    args.add_args("shared_actors", True)
    args.add_args("actor_depth", 2) 
    args.add_args("actor_width", 256)
    args.add_args("actor_acti_func", torch.nn.Tanh)
     
    # critic model settings
    args.add_args("centralised_critics", True)
    args.add_args("shared_critics", True)
    args.add_args("critic_depth", 2)
    ...
    args.add_args("obs_dims", envs.observation_spec[("agents", "observation")].shape[-1]) # add observation dims
